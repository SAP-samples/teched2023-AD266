...
2. A learning goal should be automatically created for them in SuccessFactors. (SuccessFactors OData v2 service consumption)
...
The `RegistrationServiceHandler` class would take care of registering the user (`#1` in the requirement). We would be interacting with a synthetic OpenAPI service to achieve this.
...
3. Let's now use the generated client to register for the TechEd event.
   ...
   ```
   Here we are using the generated client to register for the TechEd event.
...
4. Finally, use the generated client to register for a session in the TechEd event.   
   ...
   ```
   Here we are using the generated client to register for a session in TechEd event.
...
You've now successfully learned how to use the SAP Cloud SDK to consume a remote OpenAPI service (Registration Service) in a type safe manner.
